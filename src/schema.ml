...
let rec process_tacticals ~core seq tacs =
  match tacs with
  | [] -> ()
  | tac :: tacs ->
      let seq = process_tactical ~core seq tac in
      process_tacticals ~core seq tacs

and process_tactical ~core seq tac =
  match tac with
  | Inversion args ->
      (* inv.1 *)
  ...
  | Sync args ->
      ...
      | Pred ( t, _), Pred (t1, _ ) ->
          ...
          with  _ ->
            ...
            if vvts = [] then
              failwithf "Schema: in sync, no clauses of %s can introduce a formula of the form %s.%!"
            ...
            core aStr end
      | _ , _ -> failwith " unexpected in sync"
      end
  |  Unique args ->
      (* uni.1 *)
      let (h0,h1,h2) = try
          (get_hyp (List.nth args 0),
           get_hyp (List.nth args 1),
           get_hyp (List.nth args 2))
        with _ -> failwith "Schema: 3 arguments expected for 'unique' tactical"
      in
      ...
      | Pred(t,_),Pred(t1,_),Pred(t2,_) ->
          ...
          (if  (gi <> gi' || schName <> schName') then
             failwith "Schema: membership hypothesis should come from the same projection of the context in 'unique' tactical");
          let (arr,bids) = get_schema schName in
          let mts' = List.map (fun (a,b,tml) -> (List.nth tml (gi-1))) bids  in
          let mts  = remove_nones mts' in
          (* uni.2 *)
          let varl = pairwiseEqual te1 te2 in
          (* uni.3 *)
          fail_if_false (Unify.try_left_unify te1 te2);
          (* uni.4 *)
          let ads = instOfPats te1 mts in
          (* uni.5 *)
          let (groundVar, rel) = safeUniqueGrounds mts ads varl in
          (* uni.6 *)
          let adsHashl = List.map (fun (b,_) -> if b then "1" else "0") ads in
          ...
          | App(schNameT,schGsTl) ->
              ...
              begin
                if (List.length schOs) = arrO then
                  ...
                  else failwithf "Schema: %d arguments expected for schema %s.%!" arrD schNameD
                else failwithf "Schema: %d arguments expected for schema %s.%!" arrO schNameO
              end ;
              (* pro.2 *)
              let odPerm = List.map begin fun id -> try
              ...
              with _ ->
                (* pro.3 *)
                let (_,_,btmsO) = split3 bidsO in
                let clConsO = proClConst schOs btmsO in
                if (List.flatten clConsO) = [] then
                  failwith "Schema: Couldn't create the constraints from the original schema in projection tactic";
...
let process_tactic core st seq =
  slexbuf := Lexing.from_string st;
  try (
    let tacs = Schemaparser.command_list Schemalexer.token !slexbuf in
    process_tacticals ~core seq tacs
  ) with
  | End_of_file -> ()
  | Parsing.Parse_error ->
     eprintf "Failure to apply Ctx tactic.\nSyntax error in Schema plugin (process_tactic)\n\"%s\".\n%!" st;
     failwith "parsing"
  | e ->
     eprintf "Failure to apply Ctx tactic. \n Error %s while processing command %s in Schema plugin. \n"
             (Printexc.to_string e) st;
     raise e
;;
