...
let try_left_unify_cpairs_fully_inferred ~used ~msg t1 t2 =
  ...
  let rterms = t1 :: t2 :: (extract_terms_from_result cpairs) in
  (match cpairs with
   | None -> ()
   | Some _ ->
...
let case ~used ~sr ~clauses ~mutual ~defs ~global_support term =
  ...
  | Binding(Nabla, _, _) ->
      Option.fold ~none:[] ~some:(fun sc -> [stateless_case_to_case sc])
...
let apply ?(used_nominals=[]) ~sr term args =
  ...
  let support = hyp_support @
                  List.flatten_map (Option.fold ~some:metaterm_support ~none:[]) args in
