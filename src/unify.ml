...
let seal tyc_seal tyc_carrier eqv =
  ...
  | exception Not_found -> Hashtbl.add seals tyc_seal (tyc_carrier, eqv)
  | _, old_eqv ->
      failwithf "Seal failure: %s has already been sealed (with %s)" tyc_seal
        old_eqv
...
module Make (P : Param) = struct
  ...
  let constant tag = tag = Constant || tag = constant_like || tag = Nominal
  let variable tag = tag = instantiatable
  ...
  let lift t n =
    match observe t with Var _ -> t | DB i -> db (i + n) | _ -> susp t 0 n []
  ...
  let rec unique_bv n l =
    match l with
    | [] -> true
    | t :: rargs -> (
        match observe t with DB j when n = j -> false | _ -> unique_bv n rargs)
  ...
  let check_flex_args l fts =
    let rec aux = function
      ...
      | t :: ts -> (
          match observe t with
          | Var v when constant v.tag && v.ts > fts && unique_var v ts -> aux ts
          | DB i when unique_bv i ts -> aux ts
          | _ -> false)
    in
    aux l
  ...
  * @raise NotLLambda if the list doesn't satisfy the requirements. *)
  let ensure_flex_args l fts =
    if not (check_flex_args l fts) then raise (UnifyError NotLLambda)

  (** [bvindex bv l n] return a nonzero index iff the db index [bv] * appears in
      [l]; the index is the position from the right, representing * the DeBruijn
      index of the abstraction capturing the argument. * Arguments in the list
      are expected to be head-normalized. *)
  let rec bvindex i l n =
    match l with
    | [] -> 0
    | t :: q -> (
        match observe t with DB j when i = j -> n | _ -> bvindex i q (n - 1))

  (** [cindex c l n] return a nonzero index iff the constant [c] * appears in
      [l]; the index is the position from the right, representing * the DeBruijn
      index of the abstraction capturing the argument. * Arguments in the list
      are expected to be head-normalized. *)
  let rec cindex c l n =
    match l with
    | [] -> 0
    | t :: q -> (
        match observe t with Var c' when c = c' -> n | _ -> cindex c q (n - 1))
  ...
  * exception will be raised. *)
  let raise_and_invert ts1 ts2 a1 a2 lev =
    ...
    let rec raise_var l n =
      ...
      | t :: tl -> (
          ...
          | Var c when constant c.tag ->
              ...
              if c.ts <= ts2 then (true, db (n + lev) :: inds, t :: consts)
    ...
    let rec prune_and_raise l n =
      ...
      | a :: q, n -> (
          ...
          | Var v when constant v.tag ->
              let pruned, inds1, inds2 = prune_and_raise q (n - 1) in
              if v.ts <= ts1 then (pruned, a :: inds1, db n :: inds2)
              else
                let i = cindex v a1 l1 in
                if i = 0 then (true, inds1, inds2)
    ...
    else prune_and_raise a2 (List.length a2)
   ...
   * term). *)
  let rec prune_same_var l1 l2 j bl =
    match (l1, l2) with
    | [], [] -> []
    | [], t :: q -> (
        match observe t with
  ...
  let make_non_llambda_subst lev v1 a1 t2 =
    ...
    let rec aux lev t =
      match observe (hnorm t) with
      | Var v when variable v.tag && v <> v1 && v.ts <= v1.ts -> t
      | Var v when v.tag = Constant && v.ts <= v1.ts -> t
      | Var v when constant v.tag && v1.ts < v.ts ->
          let i = cindex v a1 n in
          if i = 0 then raise (UnifyError NotLLambda) else db (i + lev)
      | DB i ->
          if i <= lev then t
          else
            let j = bvindex (i - lev) a1 n in
            if j = 0 then raise (UnifyError NotLLambda) else db (j + lev)
      | App (h2, a2) -> app (aux lev h2) (List.map (aux lev) a2)
      | Lam (idtys2, b2) -> lambda idtys2 (aux (lev + List.length idtys2) b2)
  ...
  let reverse_bind (tyctx : (Term.id * Term.ty) list) t1 t2 =
    ...
    | App (h, ts), Var v2 -> (
        ...
        | Var v1
          when variable v1.tag && v2.ts <= v1.ts && List.for_all pruneable ts ->
  ...
  (** [makesubst tyctx h1 t2 a1 n] unifies [App (h1,a1) = t2]. * Given a term of
      the form [App (h1,a1)] where [h1] is a variable and * another term [t2],
      generate an LLambda substitution for [h1] if this is * possible, making
      whatever pruning and raising substitution that are * necessary to
      variables appearing within [t2]. * * [t2] is assumed to be in head normal
      form, [h1] and [a1] are assumed to be * dereferenced, and [n] is supposed
      to be the length of [a1]. * * Exceptions can be * raised from this code if
      a non LLambda situation is discovered or * there is failure in unification
      or a type mismatch (possible if an * a priori type checking has not been
      done) is encountered. * * The unification computation is split into two
      parts, one that * examines the top level structure of [t2] and the other
      that descends * into its nested subparts. This organization is useful
      primarily * because [h1], the variable head of the first term can appear
      at the * toplevel in t2 without sacrificing unifiability but not in a
      nested * part. *)
  let makesubst tyctx h1 t2 a1 n =
    ...
    let rec nested_subst tyctx c lev =
      ...
      | Lam (idtys, t) ->
          lambda idtys
            (nested_subst (List.rev_app idtys tyctx) t
               (lev + List.length idtys))
      | App (h2, a2) -> (
          ...
          | Var hv2 when variable hv2.tag ->
              ...
              if check_flex_args a2 hv2.ts then
                ...
                if changed then (
                  ...
                  let a2'tys = List.map (tc (List.rev_app a2ctx tyctx)) a2' in
                  let (Ty (h2tys, bty)) = tc tyctx h2 in
                  let ty =
                    Ty (a2'tys @ List.drop (List.length a2) h2tys, bty)
                  in
                  let h' = named_fresh hv1.name (min ts1 hv2.ts) ty in
                  bind h2 (lambda a2ctx (app h' a2'));
                  app h' a1')
                else if ts1 < hv2.ts then (
    ...
    let rec toplevel_subst tyctx t2 lev =
      ...
      | Lam (idtys, t2) ->
          lambda idtys
            (toplevel_subst (List.rev_app idtys tyctx) t2
               (lev + List.length idtys))
      | Var v2 when variable v2.tag ->
          if h1 = t2 then
            if n = 0 && lev = 0 then h1
            else assert false (* fail TypesMismatch *)
          else (
            if ts1 < v2.ts then Term.bind t2 (named_fresh v2.name ts1 v2.ty);
            t2)
      | App (h2, a2) -> (
          match observe h2 with
  ...
  let unifyty ty1 ty2 =
    ...
    | InstGenericTyvar (v, ty) -> raise (UnifyError (InstGenericTyvar (v, ty)))

  (** Unifying the arguments of two rigid terms with the same head, these *
      arguments being given as lists. Exceptions are raised if * unification
      fails or if there are unequal numbers of arguments; the * latter will not
      arise if type checking has been done. *)
  let rec unify_list (tyctx : (Term.id * Term.ty) list) l1 l2 =
    try List.iter2 (fun a1 a2 -> unify tyctx (hnorm a1) (hnorm a2)) l1 l2
    with Invalid_argument _ -> assert false (* fail TypesMismatch *)
    ...
    | Var v2 when constant v2.tag ->
        if v1.name = v2.name then (
          ...
          the two constants by unifying their types *)
            not (unifyty v1.ty v2.ty)
          then fail (ConstClash (cst, t2)))
        else fail (ConstClash (cst, t2))
    | Lam (idtys, t2) ->
    ...
    | Var v1, App (h2, a2) when constant v1.tag -> (
        ...
        | Var v2 when constant v2.tag ->
            if v1.name = v2.name then (
              ...
              if not (unifyty v1.ty v2.ty) then fail (ConstClash (h1, h2));
              unify_list tyctx a1 a2)
            else fail (ConstClash (h1, h2))
  ...
  and unify_var_term tyctx v1 t1 t2 =
    ...
    else if rigid_path_check v1 t2 then bind t1 t2
    else bind t1 (makesubst tyctx t1 t2 [] 0)
  ...
  let flexible_heads ~used ~sr (tys1, h1, a1) (tys2, h2, a2) =
    ...
    let projections =
      let n = a1n + (n2 - n1) in
      let bty =
        match observe_ty hv1.ty with Ty (tys, ty) -> Ty (List.drop n tys, ty)
      in
...
let left_unify ?(used = []) t1 t2 = Left.pattern_unify ~used t1 t2

let try_with_state ~fail f =
  let state = get_scoped_bind_state () in
  try f ()
  with UnifyFailure _ | UnifyError _ ->
    set_scoped_bind_state state;
    fail
