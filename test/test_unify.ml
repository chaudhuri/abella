  ...
  >::: [
         ...
         ( "[X1 a2 b3 = Y2 b3 c3]" >:: fun () ->
           ...
           let h =
             ...
             | Var { name = h; ts = 1; tag = Logic; ty } -> var Logic h 1 ty
             | _ -> assert_failure "X should match x\\y\\ H ..."
           in
           assert_term_equal
             ([ ("a", aty); ("b", bty) ] // (h ^^ [ db 2; db 1 ]))
             x;
           assert_term_equal
             ([ ("b", bty); ("c", cty) ] // (h ^^ [ a; db 2 ]))
             y );
         (* Example 6, flex-rigid case involving raise & prune relative to an
          * embedded flex term. *)
         ( "[X1 a2 b3 = c1 (Y2 b3 d3)]" >:: fun () ->
           ...
           in
           assert_term_equal
             ([ ("y", ity); ("x", ity) ] // (c ^^ [ h ^^ [ db 2; db 1 ] ]))
             x;
           assert_term_equal
             ([ ("y", ity); ("x", ity) ] // (h ^^ [ a; db 2 ]))
             y );
         (* Example 7, multiple occurences *)
         ( "[e0 (X1 a2 b3) (X1 b3 d2) = e0 (Y2 b3 c3) (e0 b3 d2)]" >:: fun () ->
           let x = var Logic "x" 1 iiity in
           let y = var Logic "y" 2 iiity in
           let a = const ~ts:2 "a" ity in
           let b = const ~ts:3 "b" ity in
           ...
           right_unify
             ...
             (e ^^ [ y ^^ [ b; c ]; e ^^ [ b; d ] ]);
           assert_term_equal
             ([ ("y", ity); ("x", ity) ] // (e ^^ [ db 2; db 1 ]))
             x;
           assert_term_equal
             ([ ("y", ity); ("x", ity) ] // (e ^^ [ a; db 2 ]))
             y );
         ...
         ( "[X1 a2 != d3 (X1 a2)]" >:: fun () ->
           ...
           assert_raises_unify_failure (fun () ->
               right_unify (x ^^ [ a ]) (d ^^ [ x ^^ [ a ] ])) );
         (* Example 11, flex-flex without raising *)
         ( "[X1 a2 b3 = Y1 b3 c3]" >:: fun () ->
           let x = var Logic "x" 1 (tyarrow [ aty; bty ] ity) in
         ...
         ( "[X1 a2 b3 = a2 (Y2 b3 c3)]" >:: fun () ->
           ...
           in
           assert_term_equal
             ([ ("a", iity); ("b", bty) ] // (db 2 ^^ [ h ^^ [ db 2; db 1 ] ]))
             x;
           assert_term_equal
             ([ ("b", bty); ("c", cty) ] // (h ^^ [ a; db 2 ]))
             y );
         (* Example 14, rigid-path failure *)
         ( "[X1 a2 b3 != d3 (Y2 b3 c3)]" >:: fun () ->
           let x = var Logic "x" 1 (tyarrow [ aty; bty ] ity) in
         ...
         ( "[T = a X, T = a Y, Y = T]" >:: fun () ->
           ...
           right_unify t (a x);
           right_unify t (a y);
           assert_raises_unify_failure (fun () -> right_unify y t) );
         ( "[x\\y\\ H1 x = x\\y\\ G2 x]" >:: fun () ->
         ...
         ( "[X1 = y2]" >:: fun () ->
           ...
           assert_raises_unify_failure (fun () -> right_unify x y) );
         (* Tests added while developing Abella *)
         ( "Saving and restoring states" >:: fun () ->
           ...
           assert_term_pprint_equal "B" a;
           assert_term_pprint_equal "B" b );
         ( "No new names for simple unification" >:: fun () ->
         ...
         ( "Loosening of LLambda restriction" >:: fun () ->
           let a = var Logic "A" 0 ity in
           let b = var Logic "B" 0 (tyarrow [ cty ] ity) in
           let c = var Logic "C" 0 cty in
           right_unify a (b ^^ [ c ]);
           assert_term_pprint_equal "B C" a );
         ( "Loosening of LLambda restriction inside of constructor" >:: fun () ->
           let a = var Logic "A" 0 aty in
         ...
         ( "Pruning should not generate a needless new name" >:: fun () ->
           let n = nominal_var "n" ity in
         ...
         ( "w\\X^0 w = Y^0 Z^0" >:: fun () ->
           ...
           assert_term_pprint_equal "Y" y;
           assert_term_pprint_equal "Z" z );
         ( "X^0 a^1 = app (Y^0 W^0 a^1) (Z^0 a^1)" >:: fun () ->
           let a = const ~ts:1 "a" aty in
         ...
         ( "p^0 (X^0 Y^0) = p^0 (Z^0 W^0)" >:: fun () ->
           ...
           match
             try_right_unify_cpairs (p ^^ [ x ^^ [ y ] ]) (p ^^ [ z ^^ [ w ] ])
           with
           | Some Res.{ cpairs = [ (a, b) ]; _ } ->
               assert_term_pprint_equal "X Y" a;
               assert_term_pprint_equal "Z W" b
           | _ -> assert_failure "Expected one conflict pair" );
         ( "p^0 (X^0 Y^0) = p^0 (z^0 w^0)" >:: fun () ->
           let p = const ~ts:0 "p" iity in
           let x = var Logic "X" 0 iity in
           let y = var Logic "Y" 0 ity in
           ...
           | Some Res.{ cpairs = l; _ } ->
               assert_failure
                 (Printf.sprintf "Expected one conflict pair, but found %d"
                    (List.length l))
           | None -> assert_failure "Unification failed" );
         ( "x1\\p^0 (X^0 x1 (Y^0 x1)) = x1\\p^0 Z^0" >:: fun () ->
           let p = const ~ts:0 "p" iity in
           let x = var Logic "X" 0 iiity in
           let y = var Logic "Y" 0 iity in
           let z = var Logic "Z" 0 ity in
         ...
         ( "R^0 N^0 = plus A^0 B^0" >:: fun () ->
           ...
           match
             left_flexible_heads ~used ~sr:Subordination.empty ([], r, [ n ])
               ([], plus, [ a; b ])
           with
           | [ t1; t2 ] ->
         ...
         ( "R^0 N^0 = x1\\x1 A^0 B^0" >:: fun () ->
           ...
           with
           | [ t1; t2 ] ->
               assert_term_pprint_equal "x1\\x2\\x2 (R1 x1 x2) (R2 x1 x2)" t1;
               assert_term_pprint_equal "x1\\x2\\x1" t2
           | ts -> assert_int_equal 2 (List.length ts) );
         ( "x1\\R^0 N^0 = x1\\x1 A^0 B^0" >:: fun () ->
           let r = var Eigen "R" 0 iity in
           let n = var Eigen "N" 0 ity in
           let a = var Eigen "A" 0 ity in
           let b = var Eigen "B" 0 ity in
           ...
           match
             left_flexible_heads ~used ~sr:Subordination.empty
               ...
               ([ ("x1", iiity) ], db 1, [ a; b ])
           with
           | [ t1 ] -> assert_term_pprint_equal "x1\\x1" t1
           | ts -> assert_int_equal 1 (List.length ts) );
         ( "R^0 X^0 b^0 = a^0 b^0" >:: fun () ->
           ...
           with
           | [ t1; t2; t3 ] ->
               assert_term_pprint_equal "x1\\x2\\a (R1 x1 x2)" t1;
               assert_term_pprint_equal "x1\\x2\\x1 (R1 x1 x2) (R2 x1 x2)" t2;
               assert_term_pprint_equal "x1\\x2\\x2" t3
           | ts -> assert_int_equal 3 (List.length ts) );
         ( "Flex-rigid subordination: R A1 B1 = sr_a_b A2 B2" >:: fun () ->
           ...
           with
           | [ t1 ] ->
               assert_term_pprint_equal "x1\\x2\\sr_a_b (R1 x1) (R2 x1 x2)" t1
           | ts -> assert_int_equal 1 (List.length ts) );
         ( "X = f\\f (X (y\\e))" >:: fun () ->
