...
let assert_raises_unify_failure f =
  ...
  with Unify.UnifyFailure _ -> ()

let assert_metaterm_unify s1 s2 =
  let ctx =
    List.map (fun id -> (id, var Logic id 0 ity)) [ "?1"; "?2"; "?3" ]
  in
  let t1 = parse_metaterm ~ctx s1 in
  let t2 = parse_metaterm ~ctx s2 in
  match meta_right_unify t1 t2 with
  | Some _ -> assert_metaterm_equal t1 t2
  | None -> assert_failure "Expected equal"
...
let tests =
  ...
  >::: [
         ...
         ( "Print smaller restricted object" >:: fun () ->
           let t = set_restriction (Smaller 1) a in
           assert_pprint_equal "{A}*" t );
         ( "Print equal restricted object" >:: fun () ->
           let t = set_restriction (Equal 1) a in
           assert_pprint_equal "{A}@" t );
         ( "Print co-smaller restricted object" >:: fun () ->
           let t = set_restriction (CoSmaller 1) a in
           assert_pprint_equal "{A}+" t );
         ( "Print co-equal restricted object" >:: fun () ->
           let t = set_restriction (CoEqual 1) a in
           assert_pprint_equal "{A}#" t );
         ( "Print second smaller restricted object" >:: fun () ->
         ...
         ( "Replace should not capture exists variables" >:: fun () ->
           let t = exists [ ("A", emptyty) ] b in
           let t' = replace_metaterm_vars [ ("B", var_a) ] t in
           assert_pprint_equal "exists A1, {A}" t' );
         ( "Cascading capture" >:: fun () ->
           let t =
             forall
               [ ("E2", emptyty) ]
               (arrow
                  (Eq (uconst "E2", uvar Eigen "E" 0))
                  (exists [ ("E1", emptyty) ] (Eq (uconst "E1", uconst "E2"))))
           in
           let t' = replace_metaterm_vars [ ("E", uvar Eigen "E1" 0) ] t in
           assert_pprint_equal "forall E2, E2 = E1 -> (exists E3, E3 = E2)" t'
         );
         ( "Print non-empty context" >:: fun () ->
         ...
         ( "Print restricted predicate" >:: fun () ->
           let p = uconst "head" ^^ [ var_a; var_b ] in
           let t = Pred (p, Smaller 1) in
           assert_pprint_equal "head A B *" t );
         ( "Print object quantifier inside of predicate" >:: fun () ->
           let t = pred (uconst "prove" ^^ [ uconst "pi" ^^ [ uconst "G" ] ]) in
           assert_pprint_equal "prove (pi G)" t );
         ( "Normalize should move all implications to the context" >:: fun () ->
           let context = Context.add (uconst "L") Context.empty in
         ...
         ( "Normalize should rename bound variables if needed" >:: fun () ->
         ...
         ( "Normalize should rename nested binders" >:: fun () ->
           ...
           let t =
             Binding
               ( Forall,
                 [ ("A", emptyty) ],
                 Arrow (pred var_a, Binding (Forall, [ ("A1", emptyty) ], eq))
               )
           in
           assert_pprint_equal "forall A1, A -> (forall A2, A1 = A2)"
             (normalize t) );
         ( "Normalize should rename nested binders (2)" >:: fun () ->
           ...
           let t =
             Binding
               ( Forall,
                 [ ("A1", emptyty) ],
                 Arrow
                   ( Eq (var_a, uvar Eigen "A1" 0),
                     Binding (Forall, [ ("A1", emptyty) ], eq) ) )
           in
           assert_pprint_equal "forall A2, A = A1 -> (forall A3, A3 = A3)"
             (normalize t) );
         ( "Normalize should rename nominals of different types" >:: fun () ->
           let t =
             pred
               (uconst "foo"
               ^^ [
         ...
         ( "Meta right unify - equality" >:: fun () ->
           assert_metaterm_unify "t1 = t2" "?1 = ?2" );
         ( "Meta right unify - pred" >:: fun () ->
           assert_metaterm_unify "foo t1" "foo ?1" );
         ( "Meta right unify - arrow" >:: fun () ->
           assert_metaterm_unify "foo t1 -> bar t2" "foo ?1 -> bar ?2" );
         ( "Meta right unify - forall" >:: fun () ->
           assert_metaterm_unify "forall A, rel1 t1 A" "forall A, rel1 ?1 A" );
         ( "Meta right unify - obj" >:: fun () ->
           assert_metaterm_unify "{p2 t2 |- p1 t1}" "{p2 t2 |- p1 ?1}" );
         ( "Meta right unify - variable capture" >:: fun () ->
           assert_raises_unify_failure (fun () ->
               assert_metaterm_unify "forall A, rel1 A A" "forall A, rel1 A ?1")
         );
         ( "Meta right unify - variable renaming" >:: fun () ->
           let t1 = parse_metaterm "forall A, foo A" in
           let t2 = parse_metaterm "forall B, foo B" in
           assert_bool "unify_failure_1" @@ (meta_right_unify t1 t2 <> None) );
         ( "Meta right unify - variable renaming (2)" >:: fun () ->
           ...
           assert_bool "unify_failure_2" @@ (meta_right_unify t1 t2 <> None);
           assert_pprint_equal "forall B, foo (iabs B)" t2 );
         ( "Fresh raised alist with subordination" >:: fun () ->
           ...
           match fresh_raised_alist ~used:[] ~sr ~tag:Eigen ~support tids with
           | [ (_, rx); (_, ry) ], [ x'; y' ] ->
